...
su_inline int
su_base_port_wait_events(su_port_t *self, su_duration_t tout)
{
  su_base_port_waiting(self);

  return self->sup_vtable->su_port_wait_events(self, tout);
}
...
void su_base_port_run(su_port_t *self)
{
  ...
  for (self->sup_running = 1; self->sup_running;) {
    ...
    su_base_port_wait_events(self, tout);
  }
}
...
void su_base_port_run_tune(su_port_t *self)
{
  ...
  for (self->sup_running = 1; self->sup_running;) {
    ...
    if (messages || timers || tout > 0)
      SU_DEBUG_9(("su_port_run(%p): %.6f: %u messages %u timers "
		  "timeout %.3f\n",
		  self, su_time_diff(su_now(), started),
		  messages, timers, tout * 1e-3));

    bedtime = su_now();

    events = su_base_port_wait_events(self, tout);

    woken = su_now();

    SU_DEBUG_9(("su_port_run(%p): %.6f: %u events slept %.6f/%.3f\n",
		self, su_time_diff(woken, started), events,
...
/** @internal Return cached timestamp */
su_time64_t
su_base_port_stamp64(su_port_t *self)
{
  su_time64_t stamp64 = self->sup_stamp64;

  if (stamp64 == 0)
    self->sup_stamp64 = stamp64 = su_stamp64();

  return stamp64;
}

/** @internal Return cached offset between monotonic timestamp and real time */
su_dur64_t
su_base_port_stamp64_offset(su_port_t *self)
{
  su_time64_t stamp64 = self->sup_stamp64;
  su_time64_t sampled = self->sup_offset.sampled;
  int need_to_stamp = stamp64 == 0;

  if (need_to_stamp)
    self->sup_stamp64 = stamp64 = su_stamp64();

  if (stamp64 > sampled + 2 * SU_E9 || sampled == 0) {
    if (!need_to_stamp)
      self->sup_stamp64 = stamp64 = su_stamp64();
    self->sup_offset.value = su_now64() - stamp64;
    self->sup_offset.sampled = stamp64;
  }

  return self->sup_offset.value;
}

/** @internal Clear cached timestamp */
void
su_base_port_waiting(su_port_t *self)
{
  self->sup_stamp64 = 0;
}
...
su_duration_t su_base_port_step(su_port_t *self, su_duration_t tout)
{
  ...
  if (su_base_port_wait_events(self, tout))
    tout = 0;
  else
  ...
  if (self->sup_head) {
    if (self->sup_vtable->su_port_getmsgs(self)) {
      ...
      if (su_base_port_wait_events(self, 0))
	tout = 0;
    }
  }
...
su_duration_t su_base_port_step(su_port_t *self, su_duration_t tout)
{
  ...
  int timers = 0, messages = 0, events = 0;
  static int once;
  static su_time_t started;
  su_time_t bedtime, woken;
  ...
  /* XXX: why isn't the timeout ignored here? */
  if (self->sup_deferrable)
    timers += su_timer_expire(&self->sup_deferrable, &tout, now);
  ...
  if (messages || timers || tout > 0)
    SU_DEBUG_9(("su_port_step(%p): %.6f: %u messages %u timers "
		"timeout %.3f\n",
		self, su_time_diff(su_now(), started),
		messages, timers, tout * 1e-3));
  ...
  SU_DEBUG_9(("su_port_step(%p): %.6f: %u events slept %.6f/%.3f\n",
	      self, su_time_diff(woken, started), events,
	      su_time_diff(woken, bedtime), tout * 1e-3));
