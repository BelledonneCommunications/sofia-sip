...
/**
 * @file su_memmem.c
 * @brief String search with Boyer-Moore algorithm
 *  
...
const char su_memmem_c_id[] = 
  "$Id: su_memmem.c,v 1.1.1.1 2005/07/20 20:35:57 kaiv Exp $";
...
bm_memmem_study(char const *needle, size_t nlen)
{
  ...
  if (fwd != NULL)
    return bm_memmem_study0(needle, nlen, fwd);
  else
    return fwd;
}
...
bm_memcasemem_study(char const *needle, size_t nlen)
{
  ...
  if (fwd != NULL)
    return bm_memcasemem_study0(needle, nlen, fwd);
  else
    return fwd;
}
